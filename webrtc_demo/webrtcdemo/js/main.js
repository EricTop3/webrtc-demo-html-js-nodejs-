...
socket.on('registerFailed', function(userName, msg) {
  console.log('register failed, userName['+userName+'] ' + msg);
  toastText.innerHTML = 'register failed,' + msg;
  startButton.disabled = false;
  callButton.disabled = true;
  hangupButton.disabled = true;
  gIsRegisterFlag = false;
});
...
function start_listen() {
	...
	trace("localUserName:"+inputName);
	if (inputName!="")
		gLocalUserName = inputName;
	else
		gLocalUserName = '0';

	socket.emit('register', gLocalUserName);
	toastText.innerHTML = 'Attempted to register:'+gLocalUserName;
	trace("Attempted to register:"+gLocalUserName);
}
//start_listen();
...
function hangup() {
  ...
  toastText.innerHTML = '挂断与['+remoteName+']的视频连接';
  stop(id);
  socket.emit('hungup', gLocalUserName, remoteName);
...
function handleRemoteHangup(remoteName) {
  ...
  toastText.innerHTML = '远程['+remoteName+']的视频连接已挂断';
